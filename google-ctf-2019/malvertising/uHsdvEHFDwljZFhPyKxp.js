...
T.e0=function(a,b){
  var c,d,e;
  return a=String(a),b=String(b),0==a.length?'':
    (c=T.f0(a.u0()),d=T.f0(b.u0().slice(0,16)),c.length,c=T.e1(c,d),e=T.longsToStr(c),e.b0())
},
  T.d0=function(a,b){
    var c,d;
  ...
  T.d1=function(a,b){
    for(var c,d,e,f=a.length,g=a[f-1],h=a[0],i=2654435769,j=Math.floor(6+52/f),k=j*i;0!=k;){
      for(d=3&k>>>2,e=f-1;e>=0;e--)g=a[e>0?e-1:f-1],c=(g>>>5^h<<2)+(h>>>3^g<<4)^(k^h)+(b[3&e^d]^g),h=a[e]-=c;k-=i
    }
    return a
  },T.f0=function(a){
  ...
  T.longsToStr=function(a){
    ...
    for(b=0;b<a.length;b++)c[b]=String.fromCharCode(255&a[b],255&a[b]>>>8,255&a[b]>>>16,255&a[b]>>>24);return c.join('')
  },
...
function dJw() {
  ...
  performance.navigation.type + performance.navigation.redirectCount + 
